@section('breadcumb','We Are Hiring')
@section('pageTitle','We-are-hiring-create')
@php
	$url = url('/admin/we-are-hirings');
@endphp
						...
						<div class="card">
							<div class="card-body">
								<div class="main-content-label mg-b-20">
									WeAreHiring {{@$WeAreHiring['id'] ? 'Edit' : 'Create'}}
								</div>
							  
								<form  data-parsley-validate="" name="faqCreate" method="POST" id="faqCreate" enctype="multipart/form-data">
									@csrf
									@if(@$WeAreHiring)
										<input type="hidden" name="id" value="{{@$WeAreHiring['id']}}}">
									@endif
									<div class="row row-sm">
										<div class="col-12">
											<div class="form-group mg-b-0">
												<label class="form-label">Title: <span class="tx-danger">*</span></label>
												<input class="form-control" name="title" placeholder="Enter Title" required="required" id="title" type="text" data-parsley-required-message="Please enter  title" value="{{@$WeAreHiring['title']}}">
										...
										<div class="col-12 mt-2">
											<div class="form-group mg-b-0">
												...
												<textarea class="form-control" name="description" placeholder="Enter description" required="required" id="description" data-parsley-required-message="Please enter your description">{{@$WeAreHiring['description']}}</textarea>
	...
	$(document).ready(function() {
		...
		$('#faqCreate').on('submit', function(e) {
			...
			if ( $(this).parsley().isValid() ) {
				 ...
				 $.ajax({
            type: "post",
            url: '{{ url("admin/we-are-hiring/store") }}',
            data: formValue,
            cache: false,
            contentType: false,
            processData: false,
            success: function(response) {
                if (response.success) {
                    ...
                    setTimeout(function(){
                        ...
                        window.location.href ='{{ url("admin/we-are-hirings") }}';
                    },2000);
                } else {
